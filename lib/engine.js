...
export function buildWeek(profile, state = {}) {
  ...
  const timePref = (schedule && schedule.time) || '';
  ...
  const timeSuffix = timePref ? ` (${timePref})` : '';
  ...
  const days = slots.map((x, idx) => {
    ...
    return { d: x.d, type: x.type, i: x.i, title, tag: tag[x.type] || 'OFF', time: (x.type === 'lift' || x.type === 'sport') ? timePref : '', items: out, meals };
  });
