...
export function buildWeek(profile, state = {}) {
  const { age, weightLb, sex, objective, daysPerWeek, sportPerWeek, equipment, injuryText = '', name = '', experience = 'consistent' } = profile;
  ...
  const days = slots.map(x => {
    let title, raw = [];
    if (x.type === 'sport') { title = 'Sport / Martial Arts'; raw = ['Your class or session', carb.sport, 'Water + electrolytes; protein within an hour after']; }
    else if (x.type === 'lift') {
      title = liftNames[x.i];
