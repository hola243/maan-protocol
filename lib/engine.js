...
// Rotating meal ideas. Index by day so the week has variety; carbs are layered on by day tag.
const MEALS = {
  breakfast: [
    'Veggie omelet with avocado',
    'Greek or coconut yogurt with berries, seeds, and a few nuts',
    'Smoked salmon with soft-scrambled eggs and tomato',
    'Protein smoothie: berries, spinach, protein powder, nut butter',
    'Savory oats topped with a fried egg',
    'Cottage cheese or a protein bowl with fruit and pumpkin seeds',
    'Turkey and sweet-potato breakfast hash with an egg',
  ],
  lunch: [
    'Big chicken salad, olive oil and lemon, no croutons',
    'Tuna and avocado over greens with cucumber',
    'Turkey or chicken lettuce wraps with shredded veg',
    'Salmon or chicken bowl over greens with a scoop of rice',
    'Lentil or white-bean soup with a side salad',
    'Steak or chicken fajita bowl: peppers, onion, guacamole',
    'Last night’s protein flaked over a big salad',
  ],
  dinner: [
    'Grilled chicken with roasted broccoli and peppers',
    'Baked salmon with asparagus and lemon',
    'Lean beef or bison burgers, no bun, over a loaded salad',
    'Shrimp stir-fry with broccoli and snap peas',
    'Sheet-pan chicken thighs with Brussels sprouts and onion',
    'Baked cod with roasted carrots and a herb drizzle',
    'Turkey meatballs in marinara over zucchini noodles',
  ],
};

function dayCarbNote(type, objective) {
  const green = objective === 'build' ? 'Load your carbs into the meal after training: rice, potatoes, or fruit (a bigger serving today).'
    : 'Put most of your carbs in the meal after training: rice, potatoes, or fruit.';
  return {
    sport: green,
    lift: 'One to two cupped hands of carbs in the meal right after you lift.',
    move: 'A cupped hand of carbs wherever it fits best today.',
    rest: 'Keep carbs to vegetables and one piece of fruit today.',
    off: 'Easy day. Carbs from vegetables and a piece of fruit.',
  }[type];
}

function dayMeals(idx, type, objective) {
  const b = MEALS.breakfast[idx % MEALS.breakfast.length];
  const l = MEALS.lunch[(idx + 2) % MEALS.lunch.length];
  const d = MEALS.dinner[(idx + 4) % MEALS.dinner.length];
  return { breakfast: b, lunch: l, dinner: d, carbNote: dayCarbNote(type, objective) };
}

const ACTIVITY_TITLES = { 'jiu jitsu': 'Jiu Jitsu', yoga: 'Yoga', swim: 'Swim', run: 'Run', other: 'Fitness Activity' };
...
// Age- and goal-appropriate starting recommendation for the training week.
// Returns recommended counts, a plain-language rationale, and a default 7-day layout
// the user can then adjust. This is what protects an older or under-training user
// from picking a week that can't reach their goal.
export function recommendSchedule(age, objective) {
  const tier = recoveryTier(age); // 'high' 50+, 'mid' 40-49, 'young' <40
  const lifts = objective === 'rebuild' ? 2 : 3;
  let activities = ({ shred: 2, lose: 3, build: 1, maintain: 2, rebuild: 1 })[objective];
  if (activities == null) activities = 2;
  if (lifts + activities > 6) activities = 6 - lifts;

  const days = { Mon: 'rest', Tue: 'rest', Wed: 'rest', Thu: 'rest', Fri: 'rest', Sat: 'rest', Sun: 'rest' };
  ['Mon', 'Wed', 'Fri', 'Sat'].slice(0, lifts).forEach(d => { days[d] = 'lift'; });
  const activityDays = ['Tue', 'Thu', 'Sat', 'Mon'].filter(d => days[d] === 'rest').slice(0, activities);
  const cardio = (objective === 'shred' || objective === 'lose') ? 'run' : 'other';
  activityDays.forEach((d, i) => { days[d] = (tier !== 'young' && i === activityDays.length - 1) ? 'yoga' : cardio; });

  const ageNote = tier === 'high'
    ? `At ${age}, strength training is the single biggest lever for staying strong, mobile, and independent. We anchor your week on ${lifts} short strength days and add ${activities} activity day${activities > 1 ? 's' : ''}, keeping at least one lower-impact (yoga or swim) to protect your joints.`
    : tier === 'mid'
      ? `In your forties, muscle and recovery both need defending. We recommend ${lifts} strength days plus ${activities} activity day${activities > 1 ? 's' : ''}, mixing something you enjoy with one easier recovery day.`
      : `We recommend ${lifts} strength days plus ${activities} activity day${activities > 1 ? 's' : ''}. You can push a little harder on the activity days at your age.`;
  const goalNote = ({
    shred: 'For fat loss, the strength days protect muscle while the activity days and your daily steps drive the deficit.',
    lose: 'For weight loss, movement matters, so we lean toward extra activity days on top of the strength work.',
    build: 'To build, the strength days do the work and recovery is precious, so activity days stay lighter.',
    maintain: 'To maintain, a steady mix of strength and activity holds your progress in place.',
    rebuild: 'Easing back in, we start with 2 strength days and add more as your check-ins show good recovery.',
  })[objective] || '';

  return {
    lifts, activities, minTraining: lifts + Math.max(0, activities - 1), minLifts: Math.max(2, lifts - 1),
    note: `${ageNote} ${goalNote}`.trim(),
    suggested: 'Good picks: something you enjoy (jiu jitsu, running, a sport) plus one lower-impact day like yoga or swimming.',
    days,
  };
}

export function buildWeek(profile, state = {}) {
  const { age, weightLb, sex, objective, daysPerWeek, sportPerWeek, equipment, injuryText = '', name = '', experience = 'consistent', activity = '', schedule = null } = profile;
  ...
  const hasSchedule = schedule && schedule.days && Object.values(schedule.days).some(v => v && v !== 'rest');

  // Build the raw Mon–Sun slot list, either from the user's chosen schedule or the legacy auto-assignment.
  let slots, effLifts, effSport;
  if (hasSchedule) {
    let liftIdx = 0;
    slots = DOW.map(d => {
      const kind = schedule.days[d] || 'rest';
      if (kind === 'lift') { const i = liftIdx % 3; liftIdx++; return { d, type: 'lift', i }; }
      if (ACTIVITY_TITLES[kind]) return { d, type: 'sport', activity: kind };
      return { d, type: 'rest' };
    });
    effLifts = slots.filter(s => s.type === 'lift').length;
    effSport = slots.filter(s => s.type === 'sport').length;
  } else {
    let lifts = isRb ? (week >= 5 && state.earnedThirdLift ? 3 : 2) : 3;
    if (experience === 'new' && week <= 4) lifts = Math.min(lifts, 2);
    const maxTraining = Math.min(daysPerWeek || 6, 6);
    if ((sportPerWeek || 0) + lifts > maxTraining) lifts = Math.max(2, maxTraining - (sportPerWeek || 0));
    const moveDays = Math.max(0, maxTraining - (sportPerWeek || 0) - lifts);
    ...
    slots = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(d => {
      if (sportSlots.includes(d)) return { d, type: 'sport', activity };
  ...
  const EX = EXERCISES[equipment] || EXERCISES.db;
  ...
  const restCheckinIdx = (() => { for (let i = slots.length - 1; i >= 0; i--) if (slots[i].type === 'rest') return i; return -1; })();

  const days = slots.map((x, idx) => {
    let title, raw = [];
    if (x.type === 'sport') {
      title = ACTIVITY_TITLES[x.activity] || 'Fitness Activity';
      raw = [`Your ${title.toLowerCase()} session`, carb.sport, 'Water + electrolytes; protein within an hour after'];
    } else if (x.type === 'lift') {
      title = liftNames[x.i];
      ...
      title = isRb ? 'Restorative Yoga' : 'Rest + Recovery';
      raw = ['Genuinely gentle: restorative yoga, a walk, or full rest', carb.rest];
      if (idx === restCheckinIdx) raw.push('2-minute check-in, it steers next week');
    } else {
      title = 'Rest';
      raw = ['No scheduled training. Get your steps and recover.'];
    }
    const { out, swapped } = applyInjuries(raw, injuries);
    swapped.forEach(s => allSwapped.add(s));
    const meals = dayMeals(idx, x.type, objective);
    return { d: x.d, type: x.type, i: x.i, title, tag: tag[x.type] || 'OFF', items: out, meals };
  });
  ...
  const EXP_NOTES = {
    new: 'You told us you are newer to training, so the block starts easy with 3 reps in reserve. Tighter margins get earned as you check in.',
  ...
  const stack = ['Creatine 5 g daily', 'Vitamin D3', 'Omega-3 fish oil ~2 g EPA/DHA with food', 'Magnesium glycinate at night']
    .concat(effSport > 0 ? ['Collagen 10–15 g + vitamin C, 45–60 min before sport or lifting'] : [])
  ...
  const grocery = [
    ...
    ['Produce', ['Salad greens (2–3 big bags)', 'Roasting vegetables: broccoli, zucchini, peppers', 'Onions and garlic', 'Avocados (4–5)', 'Lemons', effSport > 0 ? 'Bananas (post-training)' : 'Bananas', '2–3 other pieces of fruit', 'Sweet potatoes or potatoes'].join(', ')],
    ['Carbs', ['White rice', 'Oats (small container)'].concat(objective === 'build' ? ['Extra rice or pasta for the bigger carb days'] : []).join(', ')],
    ['Pantry', ['Olive oil', 'Mixed nuts'].concat(effSport > 0 ? ['Electrolyte packets'] : []).join(', ')],
    ['Supplement check', ['Creatine', 'Vitamin D3', 'Omega-3 fish oil', 'Magnesium glycinate'].concat(effSport > 0 ? ['Collagen + vitamin C'] : []).join(', ')],
  ];

  return {
    name: (name || '').split(' ')[0], week, isDeload, deloadEvery, protein, palms, lifts: effLifts, rir, rirLine,
    grocery,
